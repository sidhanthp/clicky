...
final class CompanionManager: ObservableObject {
    ...
    /// Each entry is the user's transcript and the model response.
    ...
    /// and plays the response aloud via the OpenAI speech API. The cursor stays in
    /// the spinner/processing state until TTS audio begins playing.
    /// The model response may include a [POINT:x,y:label] tag which triggers
    /// the buddy to fly to that element on screen.
    private func sendTranscriptToOpenAIWithScreenshot(transcript: String) {
        currentResponseTask?.cancel()
        openAITTSClient.stopPlayback()
        ...
        currentResponseTask = Task {
            ...
            do {
                ...
                let (fullResponseText, _) = try await openAIResponsesAPI.analyzeImageStreaming(
                ...
                // Pick the screen capture matching the requested screen number,
                ...
                if !spokenText.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty {
                    do {
                        try await openAITTSClient.speakText(spokenText)
    ...
    private func scheduleTransientHideIfNeeded() {
        ...
        transientHideTask = Task {
            ...
            while openAITTSClient.isPlaying {
    ...
    func performOnboardingDemoInteraction() {
        ...
        Task {
            do {
                ...
                // Set custom bubble text so the pointing animation uses the model
